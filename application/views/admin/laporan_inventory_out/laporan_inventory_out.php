 ...
 <!-- Content Wrapper. Contains page content -->
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
        Laporan inventory_out
          </h1>
          <ol class="breadcrumb">
            <li><a href="<?php echo base_url('index.php/admin');?>"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Laporan inventory_out</li>
          </ol>
        </section>
              ...
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Data inventory_out</h3>
				   <p>
				   <a href='<?php echo base_url('index.php/laporan_inventory_out/inventory_out_print');?>' target='_blank' ><button class='btn btn-primary'> 
				 Print preview</button>
				 </a>
				  <p>
				 
			<?php 
				if(!isset($_GET['lap_search_inv_out'])) {
				$lap_search_inv_out="";
				}
				else {
				$lap_search_inv_out=$_GET['lap_search_inv_out'];
				}
			?>
			<?php
			if ($lap_search_inv_out<>''){
			?>
				<p>Hasil pencarian dengan keyword : <i><?php echo"$lap_search_inv_out";?></i>
			<?php
			}
			?>
                  <div class="box-tools">
				   <form action='<?php echo base_url('index.php/laporan_inventory_out');?>' method='get'>
                   <div class="input-group">
                     <input type="text" name="lap_search_inv_out" value='<?php echo$this->session->userdata('lap_search_inv_out'); ?>' class="form-control input-sm pull-right" style="width: 150px;" placeholder="lap_search_inv_out"/>
                      <div class="input-group-btn">
                        <button class="btn btn-sm btn-default"><i class="fa fa-search"></i></button>
						<?php 
						
						if ($this->session->userdata('lap_search_inv_out')!=''){
						?>
						<a href='<?php echo base_url('index.php/laporan_inventory_out?ref=all');?>'><i class="btn btn-sm btn-default"><i class="fa fa-remove"></i></i></a>
						<?php 
						}
						?>
					  </div>
